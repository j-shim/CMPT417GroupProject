...
def searchA(puzzle, threshold):
    fringe = util.Stack()
    start_state = puzzle.get_start_state()
    depth = 0
    fringe.push((start_state, depth))
    expanded = util.Counter()
    remaning = False
    g = {}
    g[str(start_state)] = 0
    ...
    if puzzle.is_goal_state(node):
        return backtrace(start_state, node, parent), remaning

    expanded[str(node)] += 1

    successors = puzzle.get_successors(node)
    if node_depth < threshold:
        f_smallest = f[str(node)]
        for succ in successors:
            ...
            if g.get(str(state)) == None or tentative_g < g.get(str(state)):
                if expanded[str(state)] == 0:
                    ...
                    if f_smallest < f_value:
                        print('finding the minimum cost of all value')
                        f_smallest = f_value  # minimum cost of all values that exceeded the current threshold
        
        if f_smallest > threshold:
            return f_smallest
...
def idastar(puzzle):
    ...
    while True:
        found, remaining = searchA(puzzle, bound)
        print(found,'\n')
        if isinstance(found, float) and found != 0:
            bound = found
        else:
